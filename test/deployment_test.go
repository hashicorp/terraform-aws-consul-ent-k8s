...
var repoName string = "terraform-aws-consul-ent-k8s"
...
func TestClusterDeployment(t *testing.T) {
	...
	secretsManagerName := terraform.Output(t, prereqsTerraformOptions, "secrets_manager_name")
	...
	secondaryTfVars := fmt.Sprintf("consul_license       = \"%s\"\ncluster_name         = \"%s\"\nregion               = \"%s\"\nsecrets_manager_name = \"%s\"\n", consulLicense, secondaryClusterName, region, secretsManagerName)
	...
	// Ensure instance root volumes match recommended type
	testResults = append(testResults, assert.Equal(t, nodeGroupRootBlockDeviceVolumeType, "gp3"))
